 ...
 */
enum class IntersectionType { kIntersect = 0, kOverlap = 1, kNoIntersect = 2 };
...
class OverlapError : public IntersectError {
 public:
  OverlapError(const Vector3D& begin, const Vector3D& end, std::string error = "Segments overlap.")
      : IntersectError(error), overlap_begin(begin), overlap_end(end) {}
  OverlapError(const OverlapError& other)
      : IntersectError(other.msg), overlap_begin(other.overlap_begin), overlap_end(other.overlap_end) {}
...
class SkewError : public IntersectError {
 public:
  SkewError(const std::string error = "Segments are skew (not parallel and lie in different planes).")
...
/**
 * @note Получает на ход 2 отрезка (сегмента), возвращает точку пересечения в формате Vec3 (упрощенный формат для 3D
 * точки).
 * @exception Возвращает ошибки в случае наложения или отсутствия пересечения.
 */
