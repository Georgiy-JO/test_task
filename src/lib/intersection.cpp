...
Vector3D Intersect(const Segment3D& segment_1, const Segment3D& segment_2) {
  ...
  if (cross_prod_norm_sq < kEpsilon * kEpsilon) {  // параллельны
    ...
    if (dir_1_norm_sq < kEpsilon) {
      double tmp = (dir_2_norm_sq < kEpsilon && segment_1.GetStart() != segment_2.GetStart())? 
                -1:((segment_1.GetStart() - segment_2.GetStart()) * dir_2) / dir_2_norm_sq;

      if (tmp < 0 || tmp > 1) throw NoIntersectError();
    ...
    if (s < 0 || s > 1 || t < 0 || t > 1) throw NoIntersectError();
    return point;
  }
}
...
IntersectionType IntersectWrapper(const Segment3D& segment_1, const Segment3D& segment_2,
